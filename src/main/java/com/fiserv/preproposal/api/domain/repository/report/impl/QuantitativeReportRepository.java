...
public class QuantitativeReportRepository extends AbstractReportRepository<QuantitativeReport> {
    ...
    public String[] extractFieldsToIgnore(final String[] fields) {
        ...
        return toArray(new QuantitativeReport().extractFields().stream().filter(field -> Arrays.stream(fields).noneMatch(innerField -> innerField.equals(field))).collect(Collectors.toSet()));
    }
